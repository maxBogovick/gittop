<template>
  <div class="fixed inset-0 z-50 lg:hidden" aria-modal="true" role="dialog">
    <!-- Backdrop -->
    <div
      class="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/70"
      aria-hidden="true"
      @click="$emit('close')"
    ></div>

    <!-- Panel -->
    <div class="mobile-sidebar-panel fixed inset-y-0 left-0 w-[280px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-2xl flex flex-col border-r border-slate-200/50 dark:border-slate-700/50">
      <!-- Header -->
      <div class="flex items-center justify-between h-16 px-4 border-b border-slate-200/50 dark:border-slate-700/50">
        <div class="flex items-center gap-3 cursor-pointer" @click="navigateTo('/')">
          <div class="w-9 h-9 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20">
            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <span class="font-black text-lg tracking-tight text-slate-900 dark:text-white">GitTop</span>
        </div>
        <button
          @click="$emit('close')"
          class="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors"
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 custom-scrollbar">
        <template v-for="(group, idx) in navGroups" :key="group.label">
          <div :class="idx > 0 ? 'mt-6' : ''">
            <h3 class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              {{ group.label }}
            </h3>
            ...
            <button
              ...
              :class="[
                'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 mb-1',
