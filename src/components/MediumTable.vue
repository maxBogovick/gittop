    ...
    >
      <div class="aspect-video w-full overflow-hidden bg-slate-100 relative">
         ...
         <div class="absolute top-3 left-3 flex gap-1 flex-wrap">
            <span v-for="cat in article.categories.slice(0, 2)" :key="cat" class="bg-white/90 backdrop-blur-sm text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded text-slate-700 shadow-sm">
                {{ cat }}
            </span>
         </div>
      </div>
      <div class="p-5 flex-1 flex flex-col">
        ...
        <div class="mt-auto pt-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-medium text-slate-500">{{ article.author }}</span>
            </div>
            <span class="text-[10px] font-medium text-slate-400 uppercase tracking-widest">{{ formatDate(article.pub_date) }}</span>
        </div>
      </div>
    </div>
  </div>
