  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    >
      ...
      <div class="p-5 flex-1 flex flex-col">
        <h3 class="text-lg font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2 leading-tight mb-2">
            {{ post.title }}
        </h3>
        ...
        <div class="mt-auto pt-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <img v-if="post.author.profilePicture" :src="post.author.profilePicture" class="h-6 w-6 rounded-full ring-2 ring-white dark:ring-slate-800" />
                <span class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ post.author.name }}</span>
            </div>
            <div class="flex items-center gap-3 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                <span class="flex items-center gap-1 text-pink-500 dark:text-pink-400">
