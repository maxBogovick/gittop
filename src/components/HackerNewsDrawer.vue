    ...
    <div class="drawer-panel w-screen max-w-2xl transform transition-transform bg-white shadow-2xl flex flex-col h-full relative z-10">
      <div class="flex-shrink-0 px-6 py-6 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-20 flex items-start justify-between">
        <div class="min-w-0 pr-4">
           <h2 class="text-xl font-bold text-slate-900 leading-tight" id="slide-over-title">{{ story.title }}</h2>
           <div class="flex items-center gap-3 mt-3 text-sm text-slate-500">
              <span class="font-medium text-orange-600">{{ story.score }} points</span>
              <span>•</span>
      ...
      <div class="px-6 py-6 flex flex-col gap-6 flex-1 overflow-y-auto custom-scrollbar">
         <div v-if="story.url" class="bg-slate-50 rounded-xl p-6 border border-slate-100">
            <h3 class="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-2">Original Link</h3>
            <div class="flex items-start justify-between gap-4">
                <div class="min-w-0">
                    <p class="text-slate-900 font-medium break-all">{{ story.url }}</p>
                    <p class="text-slate-500 text-sm mt-1">{{ getHost(story.url) }}</p>
                </div>
         ...
         <div v-if="story.text" class="prose prose-slate prose-sm max-w-none text-slate-700 bg-slate-50 rounded-xl p-6 border border-slate-100" v-html="story.text"></div>

         <div class="flex flex-col items-center justify-center py-12 border-2 border-dashed border-slate-100 rounded-xl">
            ...
            <p class="text-slate-400 font-medium">Comments are hosted on Hacker News</p>
            <a :href="'https://news.ycombinator.com/item?id=' + story.id" target="_blank" class="mt-4 text-orange-600 hover:text-orange-700 font-bold text-sm">
