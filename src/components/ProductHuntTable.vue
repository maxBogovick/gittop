  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    >
      <div class="aspect-video w-full overflow-hidden bg-slate-100 dark:bg-slate-800 relative">
         ...
         <div v-else class="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500 font-bold text-4xl">
            {{ post.name.charAt(0) }}
         </div>
      </div>
      <div class="p-5 flex-1 flex flex-col">
        <h3 class="text-lg font-bold text-slate-900 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors line-clamp-1 leading-tight mb-1">
            {{ post.name }}
        </h3>
        ...
        <div class="mt-auto pt-4 flex items-center justify-between border-t border-slate-200/50 dark:border-slate-700/50">
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-1 text-orange-600 dark:text-orange-400 font-bold text-sm bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded-lg">
                    ▲ {{ post.votesCount }}
                </div>
                <div class="flex items-center gap-1 text-slate-400 dark:text-slate-500 text-sm">
                    {{ post.commentsCount }}
                </div>
            </div>
            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{{ formatDate(post.createdAt) }}</span>
        </div>
      </div>
    </div>
  </div>
