  ...
  <div class="fixed inset-0 z-50 flex justify-end" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
    ...
    <div class="drawer-panel w-screen max-w-2xl transform transition-transform bg-[var(--color-bg-secondary)] shadow-2xl flex flex-col h-full relative z-10">
      ...
      <div class="flex-shrink-0 px-6 py-4 border-b border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)]/80 backdrop-blur-md sticky top-0 z-20 flex items-start justify-between">
        <div class="flex items-start gap-4 min-w-0 pr-4">
           <div class="flex-shrink-0">
              <img v-if="listing.images && listing.images.length > 0 && listing.images[0].url_75x75" :src="listing.images[0].url_75x75" class="h-16 w-16 rounded-lg object-cover border border-[var(--color-border-primary)] shadow-sm" />
           </div>
           <div class="flex-1 min-w-0">
              <h2 class="text-lg font-bold text-[var(--color-text-primary)] leading-snug line-clamp-2" id="slide-over-title">{{ listing.title }}</h2>
              <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[var(--color-text-tertiary)] mt-1">
                 <span class="font-medium text-[var(--color-text-secondary)]">{{ listing.shop?.shop_name }}</span>
                 <span>•</span>
                 <span class="font-semibold text-green-600 dark:text-green-400">{{ formatPrice(listing.price) }}</span>
                 <span>•</span>
      ...
      <div class="px-6 py-3 bg-[var(--color-bg-tertiary)] border-b border-[var(--color-border-primary)] flex gap-2">
         <a :href="listing.url" target="_blank" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-orange-600 hover:bg-orange-700 dark:bg-orange-500 dark:hover:bg-orange-600 focus:outline-none transition-colors">
            View on Etsy
         </a>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar">
         ...
         <div v-if="listing.images && listing.images.length > 1" class="mb-8">
             <h3 class="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Gallery</h3>
             <div class="grid grid-cols-4 gap-3">
                 <div
                     v-for="(img, idx) in listing.images.slice(0, 8)"
                     :key="idx"
                     class="aspect-square rounded-lg overflow-hidden border border-[var(--color-border-primary)] cursor-zoom-in hover:ring-2 hover:ring-orange-500 transition-all"
         ...
         <div class="prose prose-slate dark:prose-invert prose-sm max-w-none text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-line">
             {{ listing.description || 'No description available.' }}
         </div>

         <div v-if="listing.tags && listing.tags.length" class="mt-8 pt-6 border-t border-[var(--color-border-primary)]">
             <h4 class="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider mb-3">Tags</h4>
             <div class="flex flex-wrap gap-2">
                 <span v-for="tag in listing.tags" :key="tag" class="px-2.5 py-1 bg-[var(--color-bg-tertiary)] text-[var(--color-text-secondary)] text-xs font-medium rounded-full">
                     {{ tag }}
                 </span>
             </div>
         </div>
