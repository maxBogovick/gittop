      ...
      <div class="flex-shrink-0 px-6 py-6 border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-20 flex items-start justify-between">
        <div class="min-w-0 pr-4">
           ...
           <div class="flex items-center gap-3 mt-4">
              ...
              <div class="text-sm">
                 <p class="font-bold text-slate-900">{{ post.author.name }}</p>
                 <p class="text-slate-500">@{{ post.author.username }} • {{ formatDate(post.publishedAt) }}</p>
              </div>
           </div>
        </div>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar">
         ...
         <div class="p-8">
             <div class="bg-blue-50 border-l-4 border-blue-500 p-6 rounded-r-xl mb-8">
                 <p class="text-slate-700 italic leading-relaxed">{{ post.brief }}</p>
             </div>

             <div class="grid grid-cols-3 gap-4 mb-12">
                <div class="bg-slate-50 p-4 rounded-xl text-center">
                    <p class="text-2xl font-bold text-slate-900">{{ post.reactionCount }}</p>
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-1">Reactions</p>
                </div>
                <div class="bg-slate-50 p-4 rounded-xl text-center">
                    <p class="text-2xl font-bold text-slate-900">{{ post.responseCount }}</p>
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-1">Comments</p>
                </div>
                <div class="bg-slate-50 p-4 rounded-xl text-center">
                    <p class="text-2xl font-bold text-slate-900">{{ post.readTimeInMinutes }}m</p>
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-1">Reading Time</p>
                </div>
             </div>
             
             <div class="pt-8 border-t border-slate-100 flex flex-col items-center">
                <p class="text-slate-500 mb-6 font-medium text-center">Hashnode provides a full reading experience for this post.</p>
                <a :href="post.url" target="_blank" class="bg-blue-600 text-white px-10 py-4 rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-lg hover:shadow-blue-500/20 active:scale-95">
