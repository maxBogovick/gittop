<template>
  <div class="fixed inset-0 z-50 flex justify-end" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
    ...
    <div class="drawer-panel w-screen max-w-2xl transform transition-transform bg-[var(--color-bg-secondary)] shadow-2xl flex flex-col h-full relative z-10">
      ...
      <!-- Header -->
      <div class="flex-shrink-0 px-6 py-4 border-b border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)]/80 backdrop-blur-md sticky top-0 z-20 flex items-center justify-between">
        <div class="flex items-center gap-3 overflow-hidden">
            <img v-if="repo.owner_avatar_url" :src="repo.owner_avatar_url" class="h-10 w-10 rounded-full ring-2 ring-[var(--color-border-secondary)]" />
            <div class="min-w-0">
              <h2 class="text-lg font-bold text-[var(--color-text-primary)] truncate" id="slide-over-title">
                {{ repo.owner }} <span class="text-[var(--color-text-muted)] font-normal">/</span> {{ repo.name }}
              </h2>
              <div class="flex items-center gap-2 text-sm text-[var(--color-text-tertiary)]">
                <span v-if="repo.primary_language" class="flex items-center gap-1">
                  <span class="w-2 h-2 rounded-full bg-[var(--color-accent-primary)]"></span>
        ...
        <button type="button" class="rounded-full p-2 text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)] transition-colors focus:outline-none" @click="$emit('close')">
      ...
      <!-- Description & Link -->
      <div class="px-6 py-4 bg-[var(--color-bg-tertiary)] border-b border-[var(--color-border-primary)]">
         <p class="text-sm text-[var(--color-text-secondary)] leading-relaxed">{{ repo.description || 'No description provided.' }}</p>
         <div class="mt-3 flex gap-2">
            <a :href="repo.repository_url" target="_blank" class="inline-flex items-center px-4 py-2 border border-[var(--color-border-primary)] text-sm font-medium rounded-lg text-[var(--color-text-secondary)] bg-[var(--color-bg-secondary)] hover:bg-[var(--color-bg-hover)] hover:text-[var(--color-accent-primary)] transition-colors shadow-sm">
      ...
      <div class="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar">
         <div v-if="loading" class="animate-pulse space-y-4">
            ...
            <div class="space-y-2 mt-8">
               <div class="h-3 bg-[var(--color-bg-tertiary)] rounded w-full"></div>
               <div class="h-3 bg-[var(--color-bg-tertiary)] rounded w-full"></div>
               <div class="h-3 bg-[var(--color-bg-tertiary)] rounded w-4/5"></div>
            </div>
         </div>
         ...
         <div v-else class="prose prose-slate dark:prose-invert prose-sm max-w-none hover:prose-a:text-[var(--color-accent-primary)]" v-html="readmeHtml"></div>
      </div>
    </div>
  </div>
</template>
